...
class CADSExtensionScript(scripts.Script):
    ...
    def cads_linear_schedule(self, t, tau1, tau2, mode):
        ...
        gamma_up = max(min(gamma_up, 1.0), 0.0)
    ...
    def on_cfg_denoiser_callback(
        ...
        if isinstance(text_cond, torch.Tensor) and isinstance(text_uncond, torch.Tensor):
            ...
            params.text_cond["crossattn"] = self.add_noise(
                text_cond["crossattn"], gamma, noise_scale, mixing_factor, rescale
            )
            params.text_uncond["crossattn"] = self.add_noise(
                text_uncond["crossattn"], gamma, noise_scale, mixing_factor, rescale
            )
            params.text_cond["vector"] = self.add_noise(
                text_cond["vector"], gamma, noise_scale, mixing_factor, rescale
            )
            params.text_uncond["vector"] = self.add_noise(
                text_uncond["vector"], gamma, noise_scale, mixing_factor, rescale
            )
        else:
